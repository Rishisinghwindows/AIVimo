...
var toastImage = UIImage(named: "user_Icon")
...
//        _ = InAppNotification.sharedInstance.showNotificationWithImage(toastImage, title: "Rocketeer", message: message)
//    }
}
...
extension UIView {
    ...
    func makeToast(message msg: String, duration: Double, position: AnyObject = HRToastPositionDefault as AnyObject) {
        let toast = viewForMessage(msg, title: nil, image: nil)
    ...
    func makeToastActivity(position pos: AnyObject, message msg: String = "") {
        ...
        activityView.center = centerPointForPosition(pos, toast: activityView)
    ...
    func hideToast(toast: UIView, force: Bool) {
        let completeClosure = { (_: Bool) -> Void in
    ...
    func centerPointForPosition(_ position: AnyObject, toast: UIView) -> CGPoint {
        if position is String {
            ...
            if position.lowercased == HRToastPositionTop {
                return CGPoint(x: viewSize.width / 2, y: (toastSize.height / 2 + HRToastVerticalMargin)+30)
        ...
        return centerPointForPosition(HRToastPositionDefault as AnyObject, toast: toast)
    }
    ...
    func viewForMessage(_ msg: String?, title: String?, image: UIImage?) -> UIView? {
        ...
        if title != nil {
            ...
            let maxSizeTitle = CGSize(width: (bounds.size.width * HRToastMaxWidth) - imageWidth, height: bounds.size.height * HRToastMaxHeight)
        ...
        if msg != nil {
            ...
            let maxSizeMessage = CGSize(width: (bounds.size.width * HRToastMaxWidth) - imageWidth, height: bounds.size.height * HRToastMaxHeight)
