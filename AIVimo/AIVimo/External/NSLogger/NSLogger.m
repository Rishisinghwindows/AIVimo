...
#define LOGGER_DIRECTORY [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:@"logs"]
...
#define LOGGER_FILENAME [NSString stringWithFormat:@"%@.txt",[NSLogger currentDay]]
...
-(void)log:(NSString *)title properties:(NSDictionary *)properties error:(BOOL)error {
   ...
   // [appendContents appendString:[NSString stringWithFormat:@"\nEVENT: \"%@\"\nERROR: %@\nPROPERTIES ¬ \n" ,error?@"TRUE":@"FALSE", title]];
     ...
     [appendContents appendString:[NSString stringWithFormat:@"Event ::: %@", title]];
